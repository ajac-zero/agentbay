...
describe("official GitHub MCP through token broker", () => {
  ...
  beforeAll(async () => {
    official = await new GenericContainer(IMAGE)
      .withCommand(["http", "--listen-host=0.0.0.0", "--port=8082", "--tools=issue_read,issue_write,pull_request_review_write,merge_pull_request"])
  ...
  it("initializes Streamable HTTP and exposes only registered tools", async () => {
    ...
    expect(listed.result.tools.map((tool: { name: string }) => tool.name)).toEqual(["issue_read", "issue_write", "merge_pull_request", "pull_request_review_write"]);
    ...
    const mergeTool = listed.result.tools.find((tool: { name: string }) => tool.name === "merge_pull_request");
    expect(mergeTool.inputSchema.required).toEqual(expect.arrayContaining(["owner", "repo", "pullNumber"]));
    expect(mergeTool.inputSchema.properties.merge_method.enum).toEqual(["merge", "squash", "rebase"]);
  });
});
