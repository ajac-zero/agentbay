...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  if (config.port !== 0 && config.metricsPort !== 0 && config.port === config.metricsPort) {
    throw new Error("PORT and DISPATCH_METRICS_PORT must be different when both are nonzero");
  }
