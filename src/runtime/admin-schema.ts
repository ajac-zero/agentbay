...
const idSchema = validatedString((value) => validateRuntimeID(value, "id"), "runtime ID").openapi({ example: "bot-default" });
const slugSchema = validatedString((value) => validateRuntimeSlug(value, "slug"), "runtime slug").openapi({ example: "default" });
const displayNameSchema = z.string().min(1).openapi({ example: "Default" });
const envVarNameSchema = z.string().min(1).openapi({ example: "TELEGRAM_BOT_TOKEN" });
...
const envVarRefSchema = z
  .object({
    ...
    valueFromEnv: envVarNameSchema,
  })
  .openapi("EnvVarRef");

const botAdapterConfigSchema = z
  .object({
    telegram: z
      .object({
        botTokenEnv: envVarNameSchema.optional(),
        secretTokenEnv: envVarNameSchema.optional(),
        userName: z.string().min(1).optional().openapi({ example: "agentbay_bot" }),
      })
      .optional(),
  })
  .openapi("BotAdapterConfig");

export const botSchema = z
  .object({
    adapters: botAdapterConfigSchema.default({}),
...
export const agentProfileSchema = z
  .object({
    claimEnv: z.array(envVarRefSchema).default([]),
    displayName: displayNameSchema,
    enabled: z.boolean(),
    id: idSchema,
...
function validatedString(validate: (value: string) => string, name: string) {
  return z.string().min(1).transform((value, context) => {
    try {
      return validate(value);
    } catch (error) {
      context.addIssue({ code: "custom", message: error instanceof Error ? error.message : `${name} is invalid` });
      return z.NEVER;
    }
  });
}
